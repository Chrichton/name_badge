defmodule NameBadge.Screen.Snake do
  ...
  defp reset_board(screen) do
    ...
    screen
    |> assign(:noop, snake: initial_snake, target: initial_target, game_over: false, points: 0)
    |> schedule_tick()
  end
  ...
  defp update_board(screen, moving_direction \\ nil) do
    ...
    cond do
      out_of_bounds?(new_head) ->
        assign(screen, :noop, game_over: true)

      snake_bites_itself?(new_head, snake) ->
        assign(screen, :noop, game_over: true)

      snake_eats_target?(new_head, target) ->
        assign(screen, :noop, snake: [new_head | snake], target: new_target, points: points + 1)

      true ->
        assign(screen, :noop, snake: Enum.drop([new_head | snake], -1))
    end
    |> then(fn screen ->
